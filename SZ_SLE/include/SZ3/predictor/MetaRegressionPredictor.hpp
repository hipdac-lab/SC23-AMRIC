...
//        free(reg_type);
    ...
    // encode_regression_coefficients(const float *reg_unpredictable_data, 
...
//        Huffman_encode_tree_and_data(2 * RegCoeffCapacity, reg_params_type, reg_count, compressed_pos);
    ...
    inline void
    regression_predict_quantize_3d(T *data_pos, const float *reg_params_pos, T *buffer, T precision,
...
//                    type_pos[j * size_z + k] = quantizer.quantize_and_overwrite(buffer_pos[j * buffer_dim1_offset + k], pred);
                    ...
                    type_pos[j * size_z + k] = quantizer.quantize_and_overwrite(cur_data, pred,
                     ...
                     data_pos[i * dim0_offset + j * dim1_offset + k] = buffer_pos[j * buffer_dim1_offset + k];
