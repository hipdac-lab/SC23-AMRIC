...
#include <cstring>
#include <vector>
#define DATASET "/level_-1/data:datatype=0"
...
void appendFileContents(const std::string& fileName, std::vector<char>& buffer) {
    std::ifstream file(fileName, std::ios::binary);
    if (!file) {
        std::cerr << "File " << fileName << " not found, skipping.\n";
        return; // Skip this file if it does not exist
    }

    // Read the contents of the file and append to the buffer
...
int main(int argc, char * argv[])
{
    ...
    /*Check that filter is not registered with the library yet*/
        ...
        int finest_level;
        FILE* fFile = fopen("../padmeta/f.txt", "r+");
        if (fFile == NULL) {
        ...
        int ncomp;
        FILE* nFile = fopen("../padmeta/ncomp.txt", "r+");
        if (nFile == NULL) {
        ...
        int nProcs;
        FILE* pFile = fopen("../padmeta/p.txt", "r+");
        if (pFile == NULL) {
        ...
        int realNprocs = 0;
        sprintf(meta_name, "../padmeta/realp_%d.txt", LEVEL);
        ...
        // {
        //         sprintf(meta_name, "../padmeta/meta_%d.txt", LEVEL);
        ...
        // } else {
                for (int i=0; i<nProcs; ++i) {
                        sprintf(meta_name, "../padmeta/meta_%d_%d.txt", LEVEL, i);
        ...
        case H5T_FLOAT:
                ...
                else //64bit: double
                {
                        ...
                        for(auto i = 0; i < procLength.size(); ++i) {
                                if (procLength[i]>-1) {
                                        procOffsets[i] = writeDataSize * ((double)(BSIZE/2)/((BSIZE/2)+1)) * ((double)(BSIZE/2)/((BSIZE/2)+1));
                        ...
                        for(auto k = 0; k < realNbEle/(((BSIZE/2)+1)*((BSIZE/2)+1)); ++k) {
                                for(auto j = 0; j < (BSIZE/2)+1; ++j) {
                                        for(auto i = 0; i < (BSIZE/2)+1; ++i) {
                                                if (i < BSIZE/2 && j <  BSIZE/2) {
                                                        noPadData.push_back(realData[i+j*((BSIZE/2)+1)+k*((BSIZE/2)+1)*((BSIZE/2)+1)]);
                                                }
                                        }
                                }
                        ...
                        std::cout << "noPadData.size(): " << noPadData.size() << std::endl;
                        ...
                        double noPadsum = 0;
                        for (int i = 0; i < noPadData.size(); ++i)
                                noPadsum+=noPadData[i];
                        std::cout << "noPadsum: "  << std::fixed << noPadsum << std::endl;
    ...
    boxDataset_1 = H5Dopen2(file, "/level_0/boxes", H5P_DEFAULT);
    ...
    // herr_t offStatus;
    // offDataset = H5Dopen2(file, "/level_1/data:offsets=0", H5P_DEFAULT);
    ...
    // offStatus = H5Dread(offDataset, offDatatype, H5S_ALL, H5S_ALL, H5P_DEFAULT, offset);
        ...
        int sp;
        sprintf(meta_name, "../padmeta/sp_%d.txt", LEVEL);
        ...
        for (int i = 0; i < nProcs; ++i) {
                ...
                fileNameStream << "../vec/proc_" << i << ".bin"; // Construct file name
                std::string fileName = fileNameStream.str();

                // Append the contents of the current file to vecBuffer
                appendFileContents(fileName, vecBuffer);
        }

        // At this point, vecBuffer contains the concatenated contents of all existing files
        std::cout << "Finished reading files. Total bytes read: " << vecBuffer.size() << std::endl;

        int vecCnt = 0;
        /*nast decomp*/
        for (int a = 0; a < boxLen_1; ++a) {
                ...
                for (int y = 0; y < (box_1[a][4]-box_1[a][1]+1)/BSIZE; ++y){
                        for (int x = 0; x < (box_1[a][3]-box_1[a][0]+1)/BSIZE; ++x){
                                for (int z = 0; z < (box_1[a][5]-box_1[a][2]+1)/BSIZE; ++z){
                                        if(vecBuffer[vecCnt] == 0) {
                                                for (int k = box_1[a][2]+z*BSIZE; k < box_1[a][2]+z*BSIZE+BSIZE; k+=2){
                                                        for (int j =box_1[a][1]+y*BSIZE; j <box_1[a][1]+y*BSIZE+BSIZE; j+=2){
                                                                for (int i = box_1[a][0]+x*BSIZE; i <box_1[a][0]+x*BSIZE+BSIZE; i+=2){
                                                                        // if(testArr[i + j*XSIZE + k*XSIZE*YSIZE] != 0) {
                                                                        testArr[i + j*XSIZE + k*XSIZE*YSIZE] = noPadData[offset+tempCnt];
                                                                        tempCnt++;
                                                                                // std::cout << offset+tempCnt << std::endl;
                                                                        // }
                                                                }
                                                        }
                                                }
    ...
    // /*stack decomp*/
        ...
        for (int k = 0; k < ZSIZE; k++) {
                for (int j = 0; j < YSIZE; j++) {
                        ...
                        // Only fill in the gaps; leave the already filled points as they are
                                if ((i % 2 == 1) || (j % 2 == 1) || (k % 2 == 1)) {
                                        // Compute indices of the nearest neighbor within the populated grid
                                        int ni = i % 2 == 0 ? i : i - 1;
                                        int nj = j % 2 == 0 ? j : j - 1;
                                        int nk = k % 2 == 0 ? k : k - 1;

                                        // Assign the value from the nearest populated neighbor
                                        testArr[i + j * XSIZE + k * XSIZE * YSIZE] = testArr[ni + nj * XSIZE + nk * XSIZE * YSIZE];
                                }
                        }
                }
        }
