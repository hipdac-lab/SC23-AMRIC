        ...
        case H5T_FLOAT:
                ...
                else //64bit: double
                {
                        ...
                        for(auto i = 0; i < procLength.size(); ++i) {
                                if (procLength[i]>-1) {
                                        procOffsets[i] = writeDataSize * ((double)BSIZE/(BSIZE+1)) * ((double)BSIZE/(BSIZE+1));
                        ...
                        for(auto k = 0; k < realNbEle/((BSIZE+1)*(BSIZE+1)); ++k) {
                                for(auto j = 0; j < BSIZE+1; ++j) {
                                        for(auto i = 0; i < BSIZE+1; ++i) {
                                                if (i < BSIZE && j < BSIZE) {
                                                        noPadData.push_back(realData[i+j*(BSIZE+1)+k*(BSIZE+1)*(BSIZE+1)]);
                                                }
                                        }
                                }
                ...
                for (int y = 0; y < (box_1[a][4]-box_1[a][1]+1)/BSIZE; ++y){
                        for (int x = 0; x < (box_1[a][3]-box_1[a][0]+1)/BSIZE; ++x){
                                for (int z = 0; z < (box_1[a][5]-box_1[a][2]+1)/BSIZE; ++z){
                                        if(vecBuffer[vecCnt] == 1) {
                                                for (int k = box_1[a][2]+z*BSIZE; k < box_1[a][2]+z*BSIZE+BSIZE; ++k){
                                                        for (int j =box_1[a][1]+y*BSIZE; j <box_1[a][1]+y*BSIZE+BSIZE; ++j){
                                                                for (int i = box_1[a][0]+x*BSIZE; i <box_1[a][0]+x*BSIZE+BSIZE; ++i){
